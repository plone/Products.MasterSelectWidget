(function($) {
    ...
    function updateSelect(field, data) {
        ...
        $('[id=archetypes-fieldname-' + field+']').find('select')
    ...
    function handleMasterVocabularyChange(event) {
        ...
        if (cache[cachekey] === undefined) {
           ...
           updateSelect(slave, cache[cachekey]);
        }        
    }
    ...
    function handleMasterToggle(event) {
        ...
        var slave = $('[id=archetypes-fieldname-' + event.data.slaveid+']');
    ...
    $.fn.bindMasterSlaveToggle = function(slaveid, action, values, master) {
        ...
        if (val.type == 'multicheckbox') {
            $items = $(this).find('input:checkbox');
            data.multivalued = true;
            $bound = $items.bind('click.masterslavetoggle' + ++guid,
                data, _anon(handleMasterToggle))
                .trigger('click.masterslavetoggle' + guid);
            $bound.each(function() {
                $item = $(this);
        ...
        if (val.type == 'radio') {
            ...
            if (val.value !== null) {
                ...
                // we must reverse the operation on last item, because it was 
                // "clicked", but shouldn't be.
                $last = $bound.last();
                $.each($last.data('events').click, function() {
                    // search for masterslavetoggle namespaced event
                    if (this.namespace.substr(0,17) == 'masterslavetoggle') {
                        // found the event - issue reverse operation 
                        val = false; // not checked
                        var action = this.data.action;
                        var slave = $('[id=archetypes-fieldname-' + this.data.slaveid+']');
                        if ($.inArray(action, ['hide', 'disable']) > -1) {
                            val = !val;
                            action = action == 'hide' ? 'show' : 'enable';
                        }
